...
int main()
{
    ...
    if(password1!=password2)
        exit(0);
    cout << endl;
    cout << "Welcome to SID Database Command Line Client :)\n\n";
    string input,temp;
    while(1)
    {
        cout << "sid> ";
        ...
        switch(getCommandCode(input))
        {
            ...
            case 1:
                if(input[13]==' ' && countWords(input)==3)
                    if(dropDatabase(toLowerCase(input.substr(14,input.size()))))
                        db = nullptr;
                else
            ...
            case 4:
                ...
                {
                    try{
                        db = Database::useDatabase(toLowerCase(input.substr(4,input.size())));
                    }catch(int){
            ...
            case 8:
                if(input[8]==' ' && countWords(input)==2)
                    if(db)
                        db->describeTable(toLowerCase(input.substr(9,input.size())));
                    else
                        cout << "No database selected ";
                else
